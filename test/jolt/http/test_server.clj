...
(defn accept-transport
  "Accept one client and return the descriptor-free transport used by tests."
  [listen-fd]
  (let [raw (c-accept listen-fd ffi/null ffi/null)]
    (when-not (neg? raw)
      (raw-transport raw))))

(defn close-listener! [listen-fd]
  (raw-close listen-fd))
...
(defn- accept-loop [listen-fd running? wrap]
  (loop []
    (let [transport (accept-transport listen-fd)]
      (cond
        (not @running?) nil
        (nil? transport) (when @running? (recur))
        :else (do
                (try
