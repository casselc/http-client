...
<#
.SYNOPSIS
  Resolve this project's graph and run one checked-in source-mode Windows gate.

.DESCRIPTION
  Invokes native Chez directly against the pinned Jolt core's CLI. It does not
  route native paths or Clojure source through a bash wrapper, and it never
  relies on the current `/dev/stdin` implementation of `joltc -`.

  Adapted from the equivalent gate in jolt-tcp, which is the transport this
  client dials through; keeping the two scripts shaped the same means a Windows
  failure can be compared directly across the layers.
#>
param(
  ...
  [Parameter(Mandatory = $true)]
  [string]$ChezExe,
  [string]$TestAlias = "-M:plaintext-test",
  [string]$GitLibsPath = "",
  [string]$ShellExe = "",
...
# Read by the test main itself, so an x86-64 process running under ARM64
# emulation fails the lane instead of reporting a green aarch64 result.
...
Write-Host "jolt-lang/http-client native Windows source gate"
